...
from api.media import normalize_media_path, public_media_url
...
class Associate:
    ...
    def Add_Associate(self):
        ...
        #-> Image Parh
        Img_path         = normalize_media_path(request.args.get('Img_path'))
        #-> Campaign id
    ...
    def View_Associate(self):
        ...
        for i in Result:
            ...
            for index, j in enumerate(i):
                j = public_media_url(j) if index == 5 else str(j)
                if( "None" in j):
                    temp.append("-")
                elif("GMT" in j):
