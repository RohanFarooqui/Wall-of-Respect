...
API_BASE_URL = os.getenv("WOR_API_URL", "http://127.0.0.1:5000").rstrip("/")


def normalize_user_image_url(value):
    ...
    if path.lower().startswith("/media/"):
        ...
        if not filename.lower().startswith("users/"):
...
class Session():
    def __init__(self,request):
        try:
            ...
            self.Img         = normalize_user_image_url(self.request.session['img'])
            if self.Img != self.request.session['img']:
                self.request.session['img'] = self.Img
