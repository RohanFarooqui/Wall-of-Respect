...
def form_upload_image(image, category):
    url = default_api_url + "/v1/media"
    files = {"image": (image.name, image, getattr(image, "content_type", "application/octet-stream"))}
    resp = requests.post(url, data={"category": category}, files=files)
